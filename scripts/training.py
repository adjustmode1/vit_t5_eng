...
from virtex.factories import (
    ...
    LRSchedulerFactory, TokenizerFactory,
)
...
import virtex.data.datasets.captioning as vdata
import pickle as pk
...
group.add_argument(
    "--loadfile", type=str, default=None,
    help="""Log training curves to tensorboard after every these many iterations
    only master process logs averaged loss values across processes.""",
)
# fmt: on
...
def main(_A: argparse.Namespace):
    ...
    # -------------------------------------------------------------------------
    loadfile = _A.loadfile
    print("load file at: ",loadfile)
    with open(loadfile+"features_train.pkl", mode='rb') as fp:
        features_train = pk.load(fp)    
    with open(loadfile+"captions_train.pkl", mode='rb') as fp:
        captions_train = pk.load(fp)    
    with open(loadfile+"features_val.pkl", mode='rb') as fp:
        features_val = pk.load(fp)    
    with open(loadfile+"captions_val.pkl", mode='rb') as fp:
        captions_val = pk.load(fp)  
    with open(loadfile+"id_train.pkl", mode='rb') as fp:
        id_train = pk.load(fp)    
    with open(loadfile+"id_val.pkl", mode='rb') as fp:
        id_val = pk.load(fp)  
    ...
    train_dataset = vdata.CaptioningDatasetTraining(id_train,features_train,captions_train,_C.DATA.PAD_INDEX)
    val_dataset = vdata.CaptioningDatasetTraining(id_val,features_val,captions_val,_C.DATA.PAD_INDEX)
    train_sampler = (
    ...
    optimizer = OptimizerFactory.from_config(_C, model.named_parameters()) # đưa param vào optimizer
    ...
    scaler = amp.GradScaler(enabled=_C.AMP) # hỗ trợ cho việc đưa dữ liệu vào model

    # Load checkpoint to resume training if specified.
    if _A.resume_from is not None: # load và gắn optimizer vào model
    ...
    for iteration in range(start_iteration + 1, _C.OPTIM.NUM_ITERATIONS + 1):
        timer.tic()
        # giai đoạn học và update param
        optimizer.zero_grad()
        batch = next(train_dataloader_iter) # lấy dữ liệu từ dataset batch có phải là 128x49x2048 không

        with amp.autocast(enabled=_C.AMP):
            output_dict = model(batch) # đưa vào model
            loss = output_dict["loss"] # nhận loss # loss là gì????
        scaler.scale(loss).backward() # scaler là gì???

        # First clip norm of gradients, and then perform optimizer step.
        scaler.unscale_(optimizer)
        torch.nn.utils.clip_grad_norm_(model.parameters(), _C.OPTIM.CLIP_GRAD_NORM) #  10.0
    ...
    torch.save(model,"/content/save_pretrain/model_virtex_train.th")
